...
import { Copy, Check, X, Search, Sparkles, Star, Zap, Flame } from 'lucide-react';
import { ALL_STYLES, TextStyleDef } from '@/data/styles';
import { transformText } from '@/lib/unicode/styleEngine';
import { CURATED_NAME_IDEAS_LIBRARY, CuratedNameIdeaItem } from '@/data/freeFireSubcategoryData';
...
export default function FreeFireNamesSubcategoryGenerator() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('lb_favorite_ff_sub_names');
      if (saved) {
  ...
  const toggleFavorite = (id: string, e?: React.MouseEvent) => {
    ...
    setFavorites((prev) => {
      ...
      try {
        localStorage.setItem('lb_favorite_ff_sub_names', JSON.stringify(updated));
      } catch {
        // Fallback
      }
      return updated;
  ...
  const filteredIdeas = useMemo(() => {
    return CURATED_NAME_IDEAS_LIBRARY.filter((item) => {
  ...
  return (
    <section className="copy-paste-tool-section" id="generador-nombres-ff">
      <div className="cp-panel">
        ...
        <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1.25rem' }}>
          ...
          >
            <Flame size={15} />
            <span>Explorar Ideas de Nombres ({CURATED_NAME_IDEAS_LIBRARY.length})</span>
          </button>
        ...
        {activeTabMode === 'custom' && (
          <div className="cp-input-block mb-6">
            <div className="cp-input-header">
              <label htmlFor="custom-ff-input" className="cp-input-title">
            ...
            <div className="cp-textarea-wrapper">
              <input
                ...
                placeholder="Escribe cualquier nombre (ej. Alex, Shadow, Dragon)..."
                maxLength={40}
              />
            </div>
          </div>
        ...
        <div className="cp-filter-bar">
          <div className="cp-search-wrapper">
            ...
            <input
              ...
              placeholder="Buscar nombre o estilo (ej. shadow, king, aesthetic, pro, gotico...)"
            />
            {searchQuery && (
              <button
                type="button"
        ...
        <div className="cp-results-header">
          <span>
            {activeTabMode === 'ideas'
              ? `Mostrando ${displayedIdeas.length} de ${filteredIdeas.length} ideas de nombres`
        ...
        {/* Results Grid - Mode 1: Curated Name Ideas */}
        {activeTabMode === 'ideas' ? (
          displayedIdeas.length === 0 ? (
            <div className="cp-empty-state" style={{ padding: '2.5rem', textAlign: 'center', color: 'var(--text-dim)' }}>
              <p>No se encontraron ideas de nombres que coincidan con tu búsqueda.</p>
              <button
                type="button"
            ...
            <div className="cp-results-grid">
              {displayedIdeas.map((item: CuratedNameIdeaItem) => {
                ...
                return (
                  <div key={item.id} className={`cp-style-card ${isCopied ? 'copied' : ''}`}>
                    <div className="cp-card-header">
                      <span className="cp-card-name">{item.category}</span>
                      <button
                        type="button"
        ...
        {((activeTabMode === 'ideas' && visibleCount < filteredIdeas.length) ||
          ...
          <div style={{ display: 'flex', justifyContent: 'center', marginTop: '2rem' }}>
            ...
            >
              Mostrar más nombres ({activeTabMode === 'ideas' ? filteredIdeas.length - visibleCount : filteredStyles.length - visibleCount} restantes)
            </button>
          </div>
        )}
      </div>
