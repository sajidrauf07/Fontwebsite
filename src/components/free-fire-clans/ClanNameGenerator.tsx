...
import { Copy, Check, X, Search, Sparkles, Star, Flame, Shield, Users } from 'lucide-react';
import { ALL_STYLES, TextStyleDef } from '@/data/styles';
import { transformText } from '@/lib/unicode/styleEngine';
import { CURATED_CLANES_IDEAS_LIBRARY, CuratedClanItem } from '@/data/freeFireClanesData';
...
export default function ClanNameGenerator() {
  ...
  const [customText, setCustomText] = useState('Shadow Squad');
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('lb_favorite_ff_clans');
      if (saved) {
  ...
  const toggleFavorite = (id: string, e?: React.MouseEvent) => {
    ...
    setFavorites((prev) => {
      ...
      try {
        localStorage.setItem('lb_favorite_ff_clans', JSON.stringify(updated));
      } catch {
        // Fallback
      }
      return updated;
  ...
  const filteredIdeas = useMemo(() => {
    return CURATED_CLANES_IDEAS_LIBRARY.filter((item) => {
  ...
  return (
    <section className="copy-paste-tool-section" id="generador-clanes-ff">
      <div className="cp-panel">
        ...
        <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
          ...
          >
            <Users size={15} />
            <span>Explorar Ideas para Clanes ({CURATED_CLANES_IDEAS_LIBRARY.length})</span>
          </button>
          ...
          >
            <Sparkles size={15} />
            <span>Transformar Nombre de Clan Personalizado</span>
          </button>
        </div>
        ...
        {activeTabMode === 'custom' && (
          <div className="cp-input-block mb-6">
            <div className="cp-input-header">
              <label htmlFor="custom-clan-input" className="cp-input-title">
                <Sparkles size={16} className="cg-icon" />
                <span>Ingresa el nombre de tu clan o equipo para transformar:</span>
              </label>
              {customText && (
                <button
                  type="button"
            ...
            <div className="cp-textarea-wrapper">
              <input
                id="custom-clan-input"
                type="text"
                className="cp-textarea"
                style={{ height: '48px', paddingTop: '10px', paddingBottom: '10px' }}
                value={customText}
                onChange={(e) => setCustomText(e.target.value)}
                placeholder="Escribe el nombre de tu clan (ej. Shadow Squad, Dark Kings)..."
                maxLength={40}
              />
            </div>
          </div>
        ...
        <div className="cp-filter-bar">
          <div className="cp-search-wrapper">
            ...
            <input
              ...
              placeholder="Buscar nombre de clan o estilo (ej. shadow, pro, insano, gotico, cortos...)"
            />
            {searchQuery && (
              <button
                type="button"
        ...
        <div className="cp-results-header">
          <span>
            {activeTabMode === 'ideas'
              ? `Mostrando ${displayedIdeas.length} de ${filteredIdeas.length} nombres de clanes`
              : `Mostrando ${displayedStyles.length} de ${filteredStyles.length} estilos para "${customText || 'Shadow Squad'}"`}
            {activeCategory !== 'Todos' ? ` en ${activeCategory}` : ''}
          </span>
        </div>

        {/* Results Grid - Mode 1: Curated Clan Ideas */}
        {activeTabMode === 'ideas' ? (
          displayedIdeas.length === 0 ? (
            <div className="cp-empty-state" style={{ padding: '2.5rem', textAlign: 'center', color: 'var(--text-dim)' }}>
              <p>No se encontraron nombres de clanes que coincidan con tu búsqueda.</p>
              <button
                type="button"
            ...
            <div className="cp-results-grid">
              {displayedIdeas.map((item: CuratedClanItem) => {
                ...
                return (
                  <div key={item.id} className={`cp-style-card ${isCopied ? 'copied' : ''}`}>
                    <div className="cp-card-header">
                      <span className="cp-card-name" style={{ color: '#10B981' }}>{item.category}</span>
                      <button
                        type="button"
          ...
          /* Results Grid - Mode 2: Custom Clan Transformations */
          displayedStyles.length === 0 ? (
            <div className="cp-empty-state" style={{ padding: '2.5rem', textAlign: 'center', color: 'var(--text-dim)' }}>
              <p>No se encontraron estilos de clan que coincidan con tu búsqueda.</p>
              <button
                type="button"
                className="cp-copy-all-btn mt-4"
                onClick={() => {
            ...
            <div className="cp-results-grid">
              {displayedStyles.map((style: TextStyleDef) => {
                const transformed = transformText(customText || 'Shadow Squad', style);
        ...
        {((activeTabMode === 'ideas' && visibleCount < filteredIdeas.length) ||
          ...
          <div style={{ display: 'flex', justifyContent: 'center', marginTop: '2rem' }}>
            ...
            >
              Mostrar más clanes ({activeTabMode === 'ideas' ? filteredIdeas.length - visibleCount : filteredStyles.length - visibleCount} restantes)
            </button>
          </div>
        )}
      </div>
