...
import { Copy, Check, X, Search, Sparkles, Star, Zap, Layers } from 'lucide-react';
import { TEXT_STYLES_COLLECTION, type TextStyleItem } from '@/data/textStyles';
...
export default function StyledTextTool() {
  const [inputText, setInputText] = useState('Texto con Estilos');
  ...
  useEffect(() => {
    try {
      const savedFavs = localStorage.getItem('lb_styled_text_favs');
      if (savedFavs) {
  ...
  const toggleFavorite = (id: string) => {
    setFavorites((prev) => {
      ...
      try {
        localStorage.setItem('lb_styled_text_favs', JSON.stringify(next));
      } catch {
        // Fallback
      }
      return next;
  ...
  const defaultText = 'Texto con Estilos';
  const textToTransform = sanitizeInput(inputText.trim() || defaultText, 250);

  const filteredStyles = useMemo(() => {
    return TEXT_STYLES_COLLECTION.filter((style) => {
  ...
  return (
    <section className="copy-paste-tool-section" id="generador-texto-estilos">
      <div className="cp-panel">
        ...
        <div className="cp-input-block">
          <div className="cp-input-header">
            <label htmlFor="styled-text-input" className="cp-input-title">
              <Sparkles size={16} className="cg-icon" />
              <span>Escribe tu texto para formatear con diferentes estilos</span>
            </label>
        ...
        <div className="cp-filter-bar">
          <div className="cp-search-wrapper">
            ...
            <input
              ...
              placeholder="Buscar estilo (ej. cursiva, elegante, negrita, aesthetic...)"
            />
            {searchQuery && (
              <button
                type="button"
          ...
          <div className="cp-tags-wrapper">
            <div className="cp-categories-scroll">
              ...
              >
                Todas ({TEXT_STYLES_COLLECTION.length})
              </button>
              <button
                type="button"
              ...
              <button
                type="button"
                className={`cp-tag-chip ${selectedCategory === 'Manuscritas' ? 'active' : ''}`}
                onClick={() => {
                  setSelectedCategory('Manuscritas');
                  setShowOnlyFavorites(false);
                }}
              >
                ✍️ Manuscritas
              ...
              <button
                type="button"
                className={`cp-tag-chip ${selectedCategory === 'Especiales' ? 'active' : ''}`}
                onClick={() => {
                  setSelectedCategory('Especiales');
                  setShowOnlyFavorites(false);
                }}
              >
                🔮 Especiales
            ...
            <button
              ...
              title="Copiar todos los resultados de texto visibles"
            >
              {copiedAll ? (
                <>
                  <Check size={14} />
        ...
        <div className="cp-results-header">
          <span>{filteredStyles.length} estilos de texto convertidos</span>
        </div>
              ...
              >
                ...
                <div className="cp-card-actions">
                  <button
                    ...
                    aria-label={isCopied ? 'Copiado' : `Copiar texto estilizado ${style.name}`}
                  >
                    {isCopied ? (
                      <>
                        <Check size={14} />
