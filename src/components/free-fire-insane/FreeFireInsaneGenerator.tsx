...
import { Copy, Check, X, Search, Sparkles, Star, Flame, Shield, Zap, Skull } from 'lucide-react';
import { ALL_STYLES, TextStyleDef } from '@/data/styles';
import { transformText } from '@/lib/unicode/styleEngine';
import { CURATED_INSANE_IDEAS_LIBRARY, CuratedInsaneIdeaItem } from '@/data/freeFireInsaneData';
...
export default function FreeFireInsaneGenerator() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('lb_favorite_ff_insane_names');
      if (saved) {
  ...
  const toggleFavorite = (id: string, e?: React.MouseEvent) => {
    ...
    setFavorites((prev) => {
      ...
      try {
        localStorage.setItem('lb_favorite_ff_insane_names', JSON.stringify(updated));
      } catch {
        // Fallback
      }
      return updated;
  ...
  const filteredIdeas = useMemo(() => {
    return CURATED_INSANE_IDEAS_LIBRARY.filter((item) => {
  ...
  return (
    <section className="copy-paste-tool-section" id="generador-nombres-insanos">
      <div className="cp-panel">
        ...
        <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
          ...
          >
            <Flame size={15} />
            <span>Explorar Nombres Insanos ({CURATED_INSANE_IDEAS_LIBRARY.length})</span>
          </button>
          ...
          >
            <Sparkles size={15} />
            <span>Transformar Mi Nombre Personalizado</span>
          </button>
        </div>
        ...
        {activeTabMode === 'custom' && (
          <div className="cp-input-block mb-6">
            <div className="cp-input-header">
              <label htmlFor="custom-insane-input" className="cp-input-title">
            ...
            <div className="cp-textarea-wrapper">
              <input
                id="custom-insane-input"
                type="text"
                className="cp-textarea"
                style={{ height: '48px', paddingTop: '10px', paddingBottom: '10px' }}
                value={customText}
                onChange={(e) => setCustomText(e.target.value)}
                placeholder="Escribe cualquier nombre (ej. Shadow, Dark, Kratos, Alex)..."
                maxLength={40}
              />
            </div>
          </div>
        ...
        <div className="cp-filter-bar">
          <div className="cp-search-wrapper">
            ...
            <input
              ...
              placeholder="Buscar nombre insano o estilo (ej. savage, dark, boss, gotico, pro...)"
            />
            {searchQuery && (
              <button
                type="button"
        ...
        <div className="cp-results-header">
          <span>
            {activeTabMode === 'ideas'
              ? `Mostrando ${displayedIdeas.length} de ${filteredIdeas.length} nombres insanos`
              : `Mostrando ${displayedStyles.length} de ${filteredStyles.length} estilos insanos para "${customText || 'Shadow'}"`}
            {activeCategory !== 'Todos' ? ` en ${activeCategory}` : ''}
          </span>
        </div>

        {/* Results Grid - Mode 1: Curated Insane Ideas */}
        {activeTabMode === 'ideas' ? (
          displayedIdeas.length === 0 ? (
            <div className="cp-empty-state" style={{ padding: '2.5rem', textAlign: 'center', color: 'var(--text-dim)' }}>
              <p>No se encontraron nombres insanos que coincidan con tu búsqueda.</p>
              <button
                type="button"
            ...
            <div className="cp-results-grid">
              {displayedIdeas.map((item: CuratedInsaneIdeaItem) => {
                ...
                return (
                  <div key={item.id} className={`cp-style-card ${isCopied ? 'copied' : ''}`}>
                    <div className="cp-card-header">
                      <span className="cp-card-name" style={{ color: '#F59E0B' }}>{item.category}</span>
                      <button
                        type="button"
          ...
          /* Results Grid - Mode 2: Custom Name Transformations */
          displayedStyles.length === 0 ? (
            <div className="cp-empty-state" style={{ padding: '2.5rem', textAlign: 'center', color: 'var(--text-dim)' }}>
              <p>No se encontraron estilos insanos que coincidan con tu búsqueda.</p>
              <button
                type="button"
                className="cp-copy-all-btn mt-4"
                onClick={() => {
        ...
        {((activeTabMode === 'ideas' && visibleCount < filteredIdeas.length) ||
          ...
          <div style={{ display: 'flex', justifyContent: 'center', marginTop: '2rem' }}>
            ...
            >
              Mostrar más nombres insanos ({activeTabMode === 'ideas' ? filteredIdeas.length - visibleCount : filteredStyles.length - visibleCount} restantes)
            </button>
          </div>
        )}
      </div>
